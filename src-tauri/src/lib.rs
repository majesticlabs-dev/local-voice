...
fn bundled_stdlib_dir(runtime_root: &Path) -> Option<PathBuf> {
    ...
    let mut candidates = fs::read_dir(&lib_dir)
        ...
        .filter_map(|entry| {
            ...
            if !path.is_dir() || !name.starts_with("python3.") {
                return None;
            }
            if path.join("encodings").join("__init__.py").is_file() {
                Some(path)
            } else {
                None
            }
...
fn bundled_pythonpath(runtime_root: &Path) -> Result<std::ffi::OsString, String> {
    let stdlib_dir = bundled_stdlib_dir(runtime_root).ok_or_else(|| {
        format!(
            "Packaged Local Voice Desktop is missing a Python standard library under {}.",
            runtime_root.join("lib").display()
        )
    })?;
    let site_packages_dir = stdlib_dir.join("site-packages");
    let mut entries = vec![stdlib_dir];
    if site_packages_dir.is_dir() {
        entries.push(site_packages_dir);
    }
    env::join_paths(entries).map_err(|err| format!("Failed to prepare bundled PYTHONPATH: {err}"))
}
...
fn resolve_service_launch_runtime(
    ...
    if !is_debug {
        ...
        return Ok(ServiceLaunchRuntime::BundledPython {
            python_binary,
            runtime_root,
        });
    }
...
fn bundled_python_runtime_complete(venv_root: &Path) -> bool {
    ...
    let has_libpython = entries.flatten().any(|entry| {
        entry
            .file_name()
            .to_str()
    ...
    has_libpython && bundled_stdlib_dir(venv_root).is_some()
}
    ...
    let line = buffer
        ...
        .find(|line| {
            !line.is_empty()
                ...
                && !line.starts_with("program name =")
                && !line.starts_with("isolated =")
                && !line.starts_with("environment =")
                && !line.starts_with("user site =")
                && !line.starts_with("safe_path =")
                && !line.starts_with("import site =")
                && !line.starts_with("is in build tree =")
                && !line.starts_with("stdlib dir =")
...
fn build_service_command(
    ...
    let mut command = match resolve_service_launch_runtime(
        ...
        ServiceLaunchRuntime::BundledPython {
            python_binary,
            runtime_root,
        } => {
...
mod tests {
    ...
    fn bundled_runtime_check_requires_libpython() {
        ...
        let venv_root = dir.join(".venv");
        fs::create_dir_all(venv_root.join("lib").join("python3.11").join("encodings"))
        ...
        .expect("write encodings placeholder");
    ...
    fn release_launch_requires_complete_bundled_runtime() {
        ...
        assert_eq!(
            ...
            Ok(ServiceLaunchRuntime::BundledPython {
                python_binary: python_path,
                runtime_root: dir.join(".bundle-venv"),
            })
        );
    ...
    #[test]
    fn service_exit_message_skips_python_frame_placeholder() {
        let dir = temp_test_dir("python-log");
...
\n\
Current thread 0x00000001f1843100 (most recent call first):\n\
  <no Python frame>\n",
        )
        .expect("write log");
